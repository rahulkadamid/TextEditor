...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(int i = 0; i < tokens.size(); i++){
			...
			// If token is a word increase numWords and numSyllables counts. Otherwise 
			// increase the numSentences count appropriately. 
			if(isWord(token)){
				numWords++;
		...
		// Check whether the final token is a terminating character (i.e. !, ? or .) or not
		// and increase numSentences appropriately. 
		int finalIndex = tokens.size() - 1;
		if (finalIndex != -1 && isWord(tokens.get(finalIndex))){
			numSentences++;
		}
	}
