...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			long timeFinishBasic = System.nanoTime();
			timeBasic = (timeFinishBasic - timeStartBasic) / 1000000000.0;
			
			long timeStartEfficient = System.nanoTime();
			for (int i = 0; i < trials; i++){
				EfficientDocument efficientDocument = new EfficientDocument(text);
				efficientDocument.getFleschScore();
			}
			long timeFinishEfficient = System.nanoTime();
			timeEfficient = (timeFinishEfficient - timeStartEfficient) / 1000000000.0;
			
			System.out.println(numToCheck + "\t" + timeBasic + "\t" + timeEfficient);
