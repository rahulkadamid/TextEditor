...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty() && !visited.contains(word2)){
			...
			List<String> nearbyWords = nw.distanceOne(curr.getWord(), true);
			
			for (String n: nearbyWords){
				if (!visited.contains(n)){
					curr.addChild(n);
					visited.add(n);
					WPTreeNode word2Node = new WPTreeNode(n, curr);
					queue.add(word2Node);
					
					if (n.equals(word2)){
						return word2Node.buildPathToRoot();
					}
				}
			}
		}
	...
	public static void main(String[] args){
		...
		List<String> path = tree.findPath("the", "zzzz");
